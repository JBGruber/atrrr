...
com_atproto_repo_create_record(repo, collection, record, .token = token)
...
```{r}
.token <- token <- req_token(user = "benguinaudeau.bsky.social", 
                             password = "")
```
...
# handle <- "janschwalbach.bsky.social"
handle <- "jbgruber.bsky.social"
